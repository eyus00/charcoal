...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  const toggleFullscreen = (e: React.MouseEvent) => {
    ...
    if (!isFullscreen) {
      // Try standard fullscreen API first (Android, Desktop)
        ...
        <div className="absolute top-0 left-0 right-0 p-4 md:p-8 flex items-start justify-between z-40 bg-gradient-to-b from-black/90 via-black/40 to-transparent pointer-events-auto" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center gap-2 md:gap-3 p-1 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-md min-w-0 max-w-[70%] sm:max-w-none">
            {onBack && (
              <button
                onClick={onBack}
                className="flex items-center justify-center w-10 h-10 md:w-12 md:h-12 text-white/70 hover:text-white hover:bg-white/10 rounded-xl transition-all active:scale-95 group p-0"
                title="Back"
              >
                <ArrowLeft className="w-5 h-5 md:w-6 md:h-6 m-auto" />
              </button>
            )}
            ...
            {showTitle && (
              <div className="flex flex-col text-left px-2 md:px-4 py-1.5 md:py-2 rounded-xl transition-all group/title min-w-0">
                <div className="flex items-center gap-2 md:gap-3 min-w-0">
                  <button
                    ...
                    className="text-white text-sm md:text-lg font-bold tracking-tight transition-colors truncate hover:text-accent active:scale-95"
                  >
                    {showTitle}
                  </button>
                  {!isMovie && seasons && (
                    <button
                      ...
                      className={cn(
                        "p-1 rounded-md transition-all flex-shrink-0 hover:bg-white/10 active:scale-95",
                        showEpisodeSelector ? "bg-accent/20 text-accent" : "bg-white/10 text-white/40"
                      )}
                    >
                ...
                {!isMovie && seasonNumber && episodeNumber && (
                  <p className="text-white/40 text-[9px] md:text-xs font-bold uppercase tracking-wider mt-0.5 truncate cursor-pointer hover:text-accent transition-colors"
                    onClick={(e) => {
                      e.stopPropagation();
                      navigate(isMovie ? `/movie/${id}` : `/tv/${id}`);
                    }}
