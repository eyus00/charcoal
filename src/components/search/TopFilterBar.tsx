...
const TopFilterBar: React.FC<TopFilterBarProps> = ({
  ...
  return (
    <div className="relative mb-6 md:mb-10 z-10 group/topbar">
      <div className="flex items-center gap-2 md:gap-4 bg-zinc-900/60 backdrop-blur-3xl border border-white/5 rounded-2xl md:rounded-full p-2 md:p-4 shadow-2xl transition-all hover:bg-zinc-900/80 hover:border-white/10">
        {/* Main Filter Toggle */}
        <button
          onClick={onOpenFilters}
          className={cn(
            "relative flex items-center gap-2 px-3 md:px-4 py-2.5 md:py-3.5 rounded-xl md:rounded-full font-black transition-all active:scale-95 flex-shrink-0",
            activeFilterCount > 0
        ...
        >
          ...
          {activeFilterCount > 0 && (
            <span className="absolute -top-1 -right-1 w-5 h-5 md:w-6 md:h-6 bg-white text-accent rounded-full flex items-center justify-center text-[8px] md:text-[9px] font-black shadow-lg">
              {activeFilterCount}
            </span>
          )}
        </button>
        ...
        <div className="relative flex-1 group/scroll min-w-0 overflow-hidden">
          <div
            ref={scrollRef}
            className="flex items-center gap-1.5 md:gap-2 overflow-x-auto scrollbar-none scroll-smooth px-1 md:px-2"
          >
            {/* High Rating Section */}
            <button
              onClick={() => onRatingChange(minRating === 7 ? 0 : 7)}
              className={cn(
                "flex-shrink-0 px-3 md:px-5 py-2 md:py-2.5 rounded-lg md:rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all border whitespace-nowrap",
                minRating === 7
            ...
            <div className="h-5 md:h-6 w-px bg-white/5 mx-0.5 md:mx-1 flex-shrink-0" />
