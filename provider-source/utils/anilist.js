...
`;
export async function getAnilistIdFromMedia(ctx, media) {
    ...
    const scored = items
        ...
        if (it.title.english)
            titles.push(it.title.english);
        if (it.title.native)
            titles.push(it.title.native);
...
`;
export async function getAnilistTitles(ctx, media) {
...
export async function getAnilistEnglishTitle(ctx, media) {
    const id = await getAnilistIdFromMedia(ctx, media);
    const res = await ctx.proxiedFetcher('', {
        baseUrl: 'https://graphql.anilist.co',
        method: 'POST',
