...
import { vidnestAlfaEmbed, vidnestBetaEmbed, vidnestDeltaEmbed, vidnestGamaEmbed, vidnestLamdaEmbed, vidnestSigmaEmbed, } from '@/providers/embeds/vidnest';
...
function isErrorResponse(result) {
    ...
    if (result.statusCode === 200 && bodyStr.trim() === 'error_wrong_ip')
        return true;
    if (result.statusCode === 200) {
        try {
            const parsed = JSON.parse(bodyStr);
            if (parsed.status === 403 && parsed.msg === 'Access Denied')
                return true;
        }
        catch {
            // Not JSON, continue
...
export async function validatePlayableStream(stream, ops, sourcererId) {
    if (SKIP_VALIDATION_CHECK_IDS.includes(sourcererId))
        return stream;
    if (stream.skipValidation)
        return stream;
    ...
    if (stream.type === 'hls') {
        ...
        if (result.statusCode < 200 || result.statusCode >= 400 || isErrorResponse(result))
            return null;
        return stream;
    }
    if (stream.type === 'file') {
...
export async function validatePlayableStreams(streams, ops, sourcererId) {
    if (SKIP_VALIDATION_CHECK_IDS.includes(sourcererId))
        return streams;
