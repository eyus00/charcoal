...
export function testEmbed(ops) {
    if (ops.testSuite.length === 0)
    ...
    describe(`embed:${ops.source.id}:${ops.embed.id}`, () => {
        ops.testSuite.forEach((test) => {
            describe(`test ${test.title}`, async () => {
                async function gatherEmbeds(providers) {
                    const results = await providers.runSourceScraper({
                        id: ops.source.id,
                        media: test,
                    });
                    if (results.embeds.length !== ops.expect.embeds)
                        throw new Error(`Embeds don't match expected amount of embeds (${ops.source.id}, ${ops.embed.id}, got ${results.embeds.length} but expected ${ops.expect.embeds})`);
                    return results.embeds;
                }
                async function runTest(providers, embedUrl) {
                    let hasError = false;
                    let streamCount = 0;
                    try {
