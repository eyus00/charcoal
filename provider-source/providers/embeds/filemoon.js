...
export const filemoonScraper = makeEmbed({
    ...
    async scrape(ctx) {
        ...
        const response = await ctx.proxiedFetcher(ctx.url, { headers });
        ...
        if (!iframeUrl)
            throw new NotFoundError('No iframe src found');
        // Get iframe content
        const iframeResponse = await ctx.proxiedFetcher(iframeUrl, { headers });
        ...
        const jsCode = iframeSoup('script')
            ...
            .html();
        if (!jsCode)
            throw new NotFoundError('No packed JS code found');
        // Unpack the JS code
        const unpacked = unpack(jsCode);
        if (!unpacked)
            throw new NotFoundError('Failed to unpack JS code');
        // Extract video URL
        const videoMatch = unpacked.match(/file:"([^"]+)"/);
        if (!videoMatch)
            throw new NotFoundError('No video URL found');
