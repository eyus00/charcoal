...
async function getDecryptedData(ctx, encryptedUrl) {
    ...
    const encryptedText = typeof encryptedRes.body === 'object' ? JSON.stringify(encryptedRes.body) : String(encryptedRes.body);
...
function makeVideasyEmbed(serverId, serverName, rank) {
    return makeEmbed({
        id: `videasy-${serverId}`,
        ...
        async scrape(ctx) {
            try {
                ...
                const validStream = sources.find((s) => {
                ...
                const subtitles = (decryptedData.subtitles || decryptedData.tracks || []).map((t) => ({
                    url: t.url || t.file,
