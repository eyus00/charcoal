...
async function createFlixHQStream(decryptedData, embedId, ctx) {
    const subtitles = (decryptedData.subtitles || []).map((s) => ({
        url: s.url,
        lang: s.lang || 'Unknown',
    ...
    const firstProxyUrl = `https://proxy2.aether.mom/proxy?url=${encodeURIComponent(decryptedData.url)}&headers=${encodeURIComponent(JSON.stringify(firstProxyHeaders))}`;
...
export const vidnestLamdaEmbed = makeEmbed({
    ...
    async scrape(ctx) {
        ...
        if (!response.data)
            throw new NotFoundError('No encrypted data found');
        ...
        if (!streams.length)
            throw new NotFoundError('Lamda: no streams');
        const englishStream = streams.find((s) => (s?.language || '').toLowerCase() === 'english');
        if (!englishStream?.url)
            throw new NotFoundError('Lamda: English stream not available');
        return {
            stream: [
                {
                    id: 'lamda-english',
...
export const vidnestAlfaEmbed = makeEmbed({
    ...
    async scrape(ctx) {
        ...
        const validSources = decryptedData.sources.filter((s) => s.url &&
            (s.isM3U8 === true ||
                s.url.includes('.m3u8') ||
                s.url.includes('/hls/') ||
        ...
        if (!validSources.length)
            throw new NotFoundError('Alfa: No valid sources found');
        const streams = [];
        const streamHeaders = vidnestHeaders;
        for (const source of validSources) {
            const isM3U8 = !source.url.includes('.txt') &&
...
export const vidnestBetaEmbed = makeEmbed({
    ...
    async scrape(ctx) {
        ...
        if (!decryptedData?.url)
            throw new NotFoundError('Beta: missing url');
...
export const vidnestGamaEmbed = makeEmbed({
    ...
    async scrape(ctx) {
        ...
        const decryptedData = await decryptVidnestData(response.data);
        if (!decryptedData?.url)
            throw new NotFoundError('Gama: missing url');
...
export const vidnestSigmaEmbed = makeEmbed({
    ...
    async scrape(ctx) {
        ...
        const hlsSources = decryptedData.sources.filter((s) => s.type === 'hls' && s.file);
        if (!hlsSources.length)
            throw new NotFoundError('Sigma: no HLS sources');
        const streamHeaders = {
...
export const vidnestCatflixEmbed = makeEmbed({
    ...
    async scrape(ctx) {
        ...
        const decryptedData = await decryptVidnestData(response.data);
        if (!decryptedData?.url)
            throw new NotFoundError('Catflix: missing url');
        const subtitles = Array.isArray(decryptedData.subtitles)
            ? decryptedData.subtitles.map((s) => ({
                url: s.url,
...
export const vidnestHexaEmbed = makeEmbed({
    ...
    async scrape(ctx) {
        ...
        const decryptedData = await decryptVidnestData(response.data);
        if (!decryptedData?.url)
            throw new NotFoundError('Hexa: missing url');
        return {
            stream: [
                {
                    id: 'hexa-auto',
...
export const vidnestDeltaEmbed = makeEmbed({
    ...
    async scrape(ctx) {
        ...
        const streams = Array.isArray(decryptedData.streams) ? decryptedData.streams : [];
        if (!streams.length)
            throw new NotFoundError('Delta: no streams');
        const hindiStream = streams.find((s) => (s?.language || '').toLowerCase() === 'hindi');
        if (!hindiStream?.url)
            throw new NotFoundError('Delta: Hindi stream not available');
        return {
            stream: [
                {
                    id: 'delta-hindi',
