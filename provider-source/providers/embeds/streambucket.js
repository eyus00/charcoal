...
export const streambucketScraper = makeEmbed({
    ...
    async scrape(ctx) {
        ...
        const postResp = await ctx.proxiedFetcher(baseUrl, {
            method: 'POST',
        ...
        const directVideoMatch = vipResp.match(/file:"(https?:\/\/[^"]+)"/) ||
        ...
        if (!iframe.length)
            throw new NotFoundError('Video iframe not found');
        const iframeUrl = iframe.attr('src');
        if (!iframeUrl)
            throw new NotFoundError('Iframe URL not found');
        // Get video page
        ...
        if (!videoMatch)
            throw new NotFoundError('Video URL not found');
