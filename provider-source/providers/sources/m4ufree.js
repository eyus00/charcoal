...
function decryptHexToUtf8(encryptedHex, key) {
...
async function fetchIframeUrl(ctx, watchPageHtml, csrfToken, cookie, referer) {
    ...
    if (ctx.media.type === 'show') {
        ...
        if (!idepisode)
            throw new NotFoundError('idepisode not found');
        // Load TV episode block
        pageHtml = await ctx.proxiedFetcher('/ajaxtv', {
            baseUrl,
            method: 'POST',
    ...
    if (!playhqData)
        throw new NotFoundError('playhq data not found');
    // Request iframe wrapper
    const iframeWrapper = await ctx.proxiedFetcher('/ajax', {
        baseUrl,
        method: 'POST',
    ...
    if (!iframeUrl)
        throw new NotFoundError('iframe src not found');
    return iframeUrl.startsWith('http') ? iframeUrl : new URL(iframeUrl, baseUrl).toString();
}
async function extractM3u8FromIframe(ctx, iframeUrl) {
    ...
    if (!idfileEnc || !idUserEnc || !domainApi)
        throw new NotFoundError('Required data not found in iframe HTML');
    ...
    const responseBody = await ctx.proxiedFetcher(`${domainApi}/playiframe`, {
        method: 'POST',
        headers: {
    ...
    if (json?.status === 1 && json?.type === 'url-m3u8-encv1' && typeof json.data === 'string') {
        ...
        if (!decryptedUrl)
            throw new NotFoundError('Failed to decrypt stream URL');
        return decryptedUrl;
    }
    throw new NotFoundError(json?.msg || 'Failed to get stream URL');
}
const comboScraper = async (ctx) => {
    ...
    const searchSlug = ctx.media.title
        ...
        .replace(/^-+|-+$/g, '')
        .replace(/Ă¢â‚¬â€œ/g, '');
    const searchPageHtml = await ctx.proxiedFetcher(`/search/${searchSlug}.html`, {
        baseUrl,
        query: {
    ...
    searchPage$('.item').each((_, el) => {
        const [, title, year] = searchPage$(el)
            .find('.imagecover a')
            .attr('title')
    ...
    if (!laravel?.value)
        throw new NotFoundError('Session cookie not found');
