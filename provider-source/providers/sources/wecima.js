...
async function comboScraper(ctx) {
    ...
    if (!contentUrl)
        throw new NotFoundError('No content URL found');
    ctx.progress(30);
    ...
    if (ctx.media.type === 'movie') {
        ...
        if (!seasonUrl)
            throw new NotFoundError(`Season ${ctx.media.season} not found`);
    ...
    if (!embedUrl)
        throw new NotFoundError('No embed URL found');
    ctx.progress(60);
    ...
    if (!videoSource)
        throw new NotFoundError('No video source found');
    ctx.progress(90);
    return {
        embeds: [],
        stream: [
