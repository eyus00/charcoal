...
async function searchAnimeFlv(ctx, title) {
    ...
    if (!results.length)
        throw new NotFoundError('No se encontró el anime en AnimeFLV');
    let animeUrl = '';
    results.each((_, el) => {
...
async function comboScraper(ctx) {
    ...
    if (!title)
        throw new NotFoundError('Falta el título');
    ...
    if (ctx.media.type === 'show') {
        ...
        if (!episode)
            throw new NotFoundError('Faltan datos de episodio');
        const episodes = await getEpisodes(ctx, animeUrl);
        const ep = episodes.find((e) => e.number === episode);
        if (!ep)
            throw new NotFoundError(`No se encontró el episodio ${episode}`);
        episodeUrl = ep.url;
    }
        ...
        if (!animeUri)
            throw new NotFoundError('No se pudo obtener el animeUri para la película');
