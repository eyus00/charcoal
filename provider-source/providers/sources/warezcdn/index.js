...
async function getEmbeds(id, servers, ctx) {
    ...
    for (const server of servers.split(',')) {
        ...
        if (url && server === 'warezcdn') {
            embeds.push({ embedId: warezcdnembedHlsScraper.id, url }, { embedId: warezcdnembedMp4Scraper.id, url }, { embedId: warezPlayerScraper.id, url });
        }
        else if (url && server === 'mixdrop')
            embeds.push({ embedId: mixdropScraper.id, url });
    }
    return { embeds };
}
