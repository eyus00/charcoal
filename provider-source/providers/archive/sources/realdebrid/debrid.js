...
async function addMagnetToRealDebrid(magnetUrl, token, ctx) {
...
async function selectAllFiles(torrentId, token, ctx) {
...
async function selectSpecificFile(torrentId, fileId, token, ctx) {
...
async function getTorrentInfo(torrentId, token, ctx) {
    const data = await ctx.proxiedFetcher.full(`${REALDEBRID_BASE_URL}/torrents/info/${torrentId}`, {
        method: 'GET',
        headers: {
            Authorization: `Bearer ${token}`,
        },
...
async function unrestrictLink(link, token, ctx) {
    console.log('Unrestricting link:', `${link.substring(0, 50)}...`);
    const data = await ctx.proxiedFetcher.full(`${REALDEBRID_BASE_URL}/unrestrict/link`, {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/x-www-form-urlencoded',
...
export async function getUnrestrictedLink(magnetUrl, token, ctx) {
    try {
        ...
        while (waitAttempts < maxWaitAttempts) {
            console.log(`Initial torrent status (wait attempt ${waitAttempts + 1}/${maxWaitAttempts}): ${torrentInfo.status}, progress: ${torrentInfo.progress}%`);
            ...
            if (torrentInfo.files && torrentInfo.files.length > 0) {
                ...
                if (mp4Files.length > 0) {
                    ...
                    if (exactMatches.length > 0) {
                        ...
                        selectedFile = exactMatches.reduce((largest, current) => current.bytes > largest.bytes ? current : largest);
                    }
                    else {
                        ...
                        if (firstWordMatches.length > 0) {
                            ...
                            selectedFile = firstWordMatches.reduce((largest, current) => current.bytes > largest.bytes ? current : largest);
                        }
                        else {
        ...
        while (attempts < maxAttempts) {
            torrentInfo = await getTorrentInfo(torrentId, token, ctx);
            console.log(`Torrent status (attempt ${attempts + 1}/${maxAttempts}): ${torrentInfo.status}, progress: ${torrentInfo.progress}%`);
            ...
            if (validCompletedStatuses.includes(torrentInfo.status) && torrentInfo.links && torrentInfo.links.length > 0) {
                ...
                if (torrentInfo.files) {
                    const mp4Files = torrentInfo.files.filter((file) => file.path.toLowerCase().endsWith('.mp4') && file.selected === 1);
                    if (mp4Files.length > 0) {
                        console.log(`Found ${mp4Files.length} MP4 files, selecting largest`);
                        const largestMp4 = mp4Files.reduce((largest, current) => current.bytes > largest.bytes ? current : largest);
