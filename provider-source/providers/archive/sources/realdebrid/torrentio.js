...
function isAcceptableQuality(torrentName) {
...
function extractQuality(torrentName) {
...
function processTorrents(streams) {
    // Filter out bad quality torrents
    const goodQualityStreams = streams.filter((stream) => isAcceptableQuality(stream.name));
    const filteredStreams = goodQualityStreams.filter((stream) => stream.title?.toLowerCase().includes('mp4') || stream.behaviorHints?.filename?.toLowerCase().includes('mp4'));
...
export async function getTorrents(ctx) {
    if (!ctx.media.imdbId) {
