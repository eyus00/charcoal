...
const universalScraper = async (ctx) => {
    ...
    if (!hash)
        throw new NotFoundError('No hash found');
    ctx.progress(10);
    ...
    if (searchRes.status !== 'y' || !searchRes.searchResult)
        throw new NotFoundError(searchRes.error);
    ...
    let id = searchRes.searchResult.find(async (x) => {
        ...
        return (compareTitle(x.t, ctx.media.title) &&
    ...
    if (ctx.media.type === 'show') {
        ...
        if (!seasonId)
            throw new NotFoundError('Season not available');
