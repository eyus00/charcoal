...
function unpackIfPacked(script) {
    try {
...
export const fileMoonScraper = makeEmbed({
    ...
    async scrape(ctx) {
        // Load initial page
        const page = await ctx.proxiedFetcher.full(ctx.url, {
        ...
        const payloadResp = iframeUrl
            ? await ctx.proxiedFetcher.full(iframeUrl, { headers: { Referer: page.finalUrl || ctx.url } })
            : page;
        ...
        if (m3u8s.length === 0)
            throw new Error('Filemoon: no m3u8 found');
        // Captions (optional)
        const captions = [];
        try {
            ...
            if (subtitlesLink) {
                const res = await ctx.proxiedFetcher(subtitlesLink);
                for (const c of res) {
