...
async function findProviderFile(providerId, type) {
...
async function updateProviderRank(filePath, newRank) {
    try {
        const content = await fs.promises.readFile(filePath, 'utf-8');
...
function getAllProviders() {
    ...
    const sources = sourceScrapers.map((scraper) => ({
        id: scraper.id,
        name: scraper.name,
        type: 'source',
        rank: scraper.rank,
    ...
    const embeds = embedScrapers.map((embed) => ({
        id: embed.id,
        name: embed.name,
        type: 'embed',
        rank: embed.rank,
...
export async function runRankManager() {
    ...
    let selectedIndex = 0;
    const swapProviders = async (index1, index2, newSelectedIndex) => {
